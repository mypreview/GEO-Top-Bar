    ...
    api.controlConstructor['background-image'] = api.UploadControl.extend({
        ...
        ready: function() {
            ...
            control.container.on('click keydown', '.remove-button',
                function() {
                    jQuery('.mypreview-geo-top-bar-background-image-fields').hide();
                }
            );

            control.container.on('change', '.mypreview-geo-top-bar-background-image-repeat select',
                function() {
                    control.settings['repeat'].set(jQuery(this).val());
                }
            );

            control.container.on('change', '.mypreview-geo-top-bar-background-image-size select',
                function() {
                    control.settings['size'].set(jQuery(this).val());
                }
            );

            control.container.on('change', '.mypreview-geo-top-bar-background-image-attach select',
                function() {
                    control.settings['attach'].set(jQuery(this).val());
                }
            );

            control.container.on('change', '.mypreview-geo-top-bar-background-image-position select',
                function() {
